...
#시나리오
 - Youtube 동영상링크를 공유하는 게시판 서비스가 있다고 가정한다. 이 때, 링크의 썸네일을 보여주고자 한다.
 - 가상의 게시판 서비스는 입력된 (1) 동영상 링크와 (2) 저장할 썸네일 이미지 파일을 Json형태로 Queue로 전송한다.
 ...
 - Queue에 입력된 Youtube 링크가 없는 경우, `error`라고 적힌 텍스트파일을 생성한다.
...
#설치 및 실행방법
 ...
 1. 서비스 개설
   ...
   2. Azure Storage 생성
 2. 설치
   1. Azure WebSites에 git deployment 설정을 한 후 본 솔루션를 Push한다.
 ...
 3. ConnectionString 설정
   ...
   2. Azure WebSites > Configuration > ConnectionString에서 이름 `AzureWebJobsDashboard`를 생성한 후 값 `DefaultEndpointsProtocol=https;AccountName=tagboa;AccountKey=[STORAGE_ACCESS_KEY]`형태로 키를 입력. `[STORAGE_ACCESS_KEY]`에 Storage 앞서 복사한 액세스키를 입력한다. 세번째 콤보박스는 `사용자 지정`으로 선택함.
